...
<body>
<form action="ServletSearchXHJP" method="post" id="frm">
	<div class="div_list">
		设备名称：<input type="text" class="form-control form1"   name="XHJPName" value="${xhjp.getXHJPName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="submit" class="btn btn-primary" value="查询" />
	</div>
</form>
<form action="ServletFindAllXHJP" method="post" id="frm1">
	<div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<c:forEach var="list" items="${list.pbjs }">
				<tr>
					<td>${list.getXHJPName() }</td>
					<td>${list.getXHJPDate() }</td>
					<td><a href="ServletDeleteXHJP?XHJPID=${list.getXHJPID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
				</tr>
			</c:forEach>
			<tr>
				<td colspan="7">
