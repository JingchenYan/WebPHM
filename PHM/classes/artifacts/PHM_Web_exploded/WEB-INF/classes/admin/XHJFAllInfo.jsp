...
<body>
<form action="ServletSearchXHJF" method="post" id="frm">
	<div class="div_list">
		设备名称：<input type="text" class="form-control form1"   name="XHJFName" value="${xhjf.getXHJFName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="submit" class="btn btn-primary" value="查询" />
	</div>
</form>
<form action="ServletFindAllXHJF" method="post" id="frm1">
	<div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<c:forEach var="list" items="${list.pbjs }">
				<tr>
					<td>${list.getXHJFName() }</td>
					<td>${list.getXHJFDate() }</td>
					<td>${list.getXHJRDate() }</td>
					<td>${list.getXHJPop() }</td>
					<td>${list.getXHJFault() }</td>
					<td><a href="ServletSelectXHJF?XHJFID=${list.getXHJFID() }">修改</a>&nbsp;&nbsp;&nbsp;<a href="ServletDeleteXHJF?XHJFID=${list.getXHJFID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
