...
<body>
<form action="ServletSearchCTCF" method="post" id="frm">
	<div class="div_list">
		设备名称：<input type="text" class="form-control form1"   name="CTCFName" value="${ctcf.getCTCFName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="submit" class="btn btn-primary" value="查询" />
	</div>
</form>
<form action="ServletFindAllCTCF" method="post" id="frm1">
	<div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<c:forEach var="list" items="${list.pbjs }">
				<tr>
					<td>${list.getCTCFName() }</td>
					<td>${list.getCTCFDate() }</td>
					<td>${list.getCTCRDate() }</td>
					<td>${list.getCTCPop() }</td>
					<td>${list.getCTCFault() }</td>
					<td><a href="ServletSelectCTCF?CTCFID=${list.getCTCFID() }">修改</a>&nbsp;&nbsp;&nbsp;<a href="ServletDeleteCTCF?CTCFID=${list.getCTCFID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
