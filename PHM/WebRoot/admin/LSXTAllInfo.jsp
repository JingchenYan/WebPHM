...
function page(s){
  ...
  <body>
  <form action="ServletSearchLSXT" method="post" id="frm">
	  <div class="div_list">
		  设备名称：<input type="text" class="form-control form1"   name="LSXTName" value="${lsxt.getLSXTName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		  <input type="submit" class="btn btn-primary" value="查询" />
	  </div>
  </form>
    <form action="ServletFindAllLSXT" method="post" id="frm1">
    <div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<tr>
				<td>${list.getLSXTName() }</td>
				<td>${list.getLSXTMDate() }</td>
				<td>${list.getLSXTIDate() }</td>
				<td>${list.getLSXTHealth() }</td>
				<td><a href="ServletSearchLSXTF?LSXTFName=${list.getLSXTName() }">查看</a></td>
				<td><a href="ServletSelectLSXT?LSXTID=${list.getLSXTID() }">修改</a>&nbsp;&nbsp;&nbsp;<a href="ServletDeleteLSXT?LSXTID=${list.getLSXTID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
