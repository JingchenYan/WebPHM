...
function page(s){
  ...
  <body>
  <form action="ServletSearchGDQD" method="post" id="frm">
	  <div class="div_list">
		  设备名称：<input type="text" class="form-control form1"   name="GDQDName" value="${gdqd.getGDQDName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		  <input type="submit" class="btn btn-primary" value="查询" />
	  </div>
  </form>
    <form action="ServletFindAllGDQD" method="post" id="frm1">
    <div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<tr>
				<td>${list.getGDQDName() }</td>
				<td>${list.getGDQDMDate() }</td>
				<td>${list.getGDQDIDate() }</td>
				<td>${list.getGDQDHealth() }</td>
				<td><a href="ServletSearchGDQDF?GDQDFName=${list.getGDQDName() }">查看</a></td>
				<td><a href="ServletSelectGDQD?GDQDID=${list.getGDQDID() }">修改</a>&nbsp;&nbsp;&nbsp;<a href="ServletDeleteGDQD?GDQDID=${list.getGDQDID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
