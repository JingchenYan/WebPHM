...
function page(s){
  ...
  <body>
  <form action="ServletSearchCTC" method="post" id="frm">
	  <div class="div_list">
		  设备名称：<input type="text" class="form-control form1"   name="CTCName" value="${ctc.getCTCName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		  <input type="submit" class="btn btn-primary" value="查询" />
	  </div>
  </form>
    <form action="ServletFindAllCTC" method="post" id="frm1">
    <div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<tr>
				<td>${list.getCTCName() }</td>
				<td>${list.getCTCMDate() }</td>
				<td>${list.getCTCIDate() }</td>
				<td>${list.getCTCHealth() }</td>
				<td><a href="ServletSearchCTCF?CTCFName=${list.getCTCName() }">查看</a></td>
				<td><a href="ServletSelectCTC?CTCID=${list.getCTCID() }">修改</a>&nbsp;&nbsp;&nbsp;<a href="ServletDeleteCTC?CTCID=${list.getCTCID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
