...
function page(s){
  ...
  <body>
  <form action="ServletSearchYDQ" method="post" id="frm">
	  <div class="div_list">
		  设备名称：<input type="text" class="form-control form1"   name="YDQName" value="${ydq.getYDQName()}" />&nbsp;&nbsp;&nbsp;&nbsp;
		  <input type="submit" class="btn btn-primary" value="查询" />
	  </div>
  </form>
    <form action="ServletFindAllYDQ" method="post" id="frm1">
    <div class="div_list1 panel panel-default">
		...
		<table class="table panel-body" id="tableClass">
			...
			<tr>
				<td>${list.getYDQName() }</td>
				<td>${list.getYDQMDate() }</td>
				<td>${list.getYDQIDate() }</td>
				<td>${list.getYDQHealth() }</td>
				<td><a href="ServletSearchYDQF?YDQFName=${list.getYDQName() }">查看</a></td>
				<td><a href="ServletSelectYDQ?YDQID=${list.getYDQID() }">修改</a>&nbsp;&nbsp;&nbsp;<a href="ServletDeleteYDQ?YDQID=${list.getYDQID() }" onclick="return confirm('是否确认删除？')">删除</a></td>
